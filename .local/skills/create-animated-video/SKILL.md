...
- **Do not modify `client/src/lib/video/hooks.ts`.** The hook manages the recording lifecycle -- calling `window.startRecording?.()` on mount and `window.stopRecording?.()` after the first complete pass. The recording/export pipeline depends on both calls firing at the correct time with the correct implementation. Do not rewrite, refactor, or replace the hook. If you need different scene behavior, change your `SCENE_DURATIONS` or scene components -- not the hook itself.
...
- **Do not modify `client/src/lib/video/hooks.ts`.** Do not rewrite or replace the `useVideoPlayer` hook -- the recording/export pipeline depends on its exact implementation. The hook already handles `window.startRecording?.()` on mount and `window.stopRecording?.()` after the first full pass.
</video_structure>
...
**`useVideoPlayer` hook** (from `@/lib/video`): Use `useVideoPlayer({ durations: SCENE_DURATIONS })`. Returns `{ currentScene }`. Handles recording API (`window.startRecording` and `window.stopRecording`), scene advancement, and looping automatically. **Do not modify `client/src/lib/video/hooks.ts`** -- see `<video_structure>` for details.
...
- Static assets you create go in `client/public/` -- served at root URL
  ...
  - `client/public/assets/hero.jpg` → `/assets/hero.jpg`
...
1. Create scene files in `client/src/components/video/video_scenes/` -- `Scene1.tsx` through `Scene5.tsx` for example
...
- Use `useVideoPlayer` from `@/lib/video` for scene management (see `<video_structure>` for the pattern). **Do not modify `client/src/lib/video/hooks.ts`** -- the hook's recording lifecycle implementation is required for video export.
- The main video component is `client/src/components/video/VideoTemplate.tsx` -- this is where `SCENE_DURATIONS`, `useVideoPlayer`, and `AnimatePresence` live
- Each scene goes in its own file under `client/src/components/video/video_scenes/` named `Scene1.tsx` through `Scene5.tsx` for example. Export the scene component as a named export matching the filename (e.g., `export function Scene1() { ... }`)
...
- `task`: "Finalize video polish -- verify all content fits within the 16:9 frame without cutoff, remove any responsive breakpoints, and confirm the loop is clean. Follow the instructions in .local/skills/create-animated-video/references/finalize_playback.md exactly."
- `relevant_files`: [".local/skills/create-animated-video/references/finalize_playback.md"]
